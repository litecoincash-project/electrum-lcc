...
from electrum_lcc.util import FileImportFailed, FileExportFailed
from electrum_lcc.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
...
def export_meta_gui(electrum_window, title, exporter):
    ...
    filename = electrum_window.getSaveFileName(_("Select file to save your {}").format(title),
                                               'electrum-lcc_{}.json'.format(title), filter_)
    if not filename:
        return
    try:
        exporter(filename)
