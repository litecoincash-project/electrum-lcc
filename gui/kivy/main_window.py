...
import electrum_lcc as electrum
from electrum_lcc.bitcoin import TYPE_ADDRESS
from electrum_lcc import WalletStorage, Wallet
from electrum_lcc_gui.kivy.i18n import _
from electrum_lcc.paymentrequest import InvoiceStore
from electrum_lcc.util import profiler, InvalidPassword
from electrum_lcc.plugins import run_hook
from electrum_lcc.util import format_satoshis, format_satoshis_plain
from electrum_lcc.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
...
# the data
Cache.register('electrum_lcc_widgets', timeout=0)
...
Factory.register('TabbedCarousel', module='electrum_lcc_gui.kivy.uix.screens')
...
from electrum_lcc.util import base_units
...
class ElectrumWindow(App):
    ...
    def on_new_intent(self, intent):
        if intent.getScheme() != 'litecoincash':
            return
    ...
    def _get_bu(self):
        return self.electrum_config.get('base_unit', 'LCC')
    ...
    def on_qr(self, data):
        from electrum_lcc.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        if data.startswith('litecoincash:'):
            self.set_URI(data)
            return
        # try to decode transaction
        from electrum_lcc.transaction import Transaction
        from electrum_lcc.util import bh2u
        try:
    ...
    def show_pr_details(self, req, status, is_invoice):
        from electrum_lcc.util import format_time
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='electrum_lcc_gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='electrum_lcc_gui.kivy.uix.qrcodewidget')

        # preload widgets. Remove this if you want to load the widgets on demand
        #Cache.append('electrum_lcc_widgets', 'AnimatedPopup', Factory.AnimatedPopup())
        ...
        self.icon = "icons/electrum-lcc.png"
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Electrum-LCC', message,
                            app_icon=icon, app_name='Electrum-LCC')
        except ImportError:
