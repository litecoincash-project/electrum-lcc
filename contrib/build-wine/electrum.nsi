...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-LCC"
  !define PRODUCT_WEB_SITE "https://github.com/litecoincash-project/electrum-lcc"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-lcc-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-lcc\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\litecoincash" "" "URL:litecoincash Protocol"
  WriteRegStr HKCU "Software\Classes\litecoincash" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\litecoincash" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\litecoincash\shell\open\command" "" "$\"$INSTDIR\electrum-lcc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\litecoincash"
