...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-lcc-macos
	cd /tmp/electrum-lcc-macos
...
rm -rf /tmp/electrum-lcc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-lcc-macos/image/
cp -r $1 /tmp/electrum-lcc-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-LCC_uncompressed.dmg \
    /tmp/electrum-lcc-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-LCC_uncompressed.dmg electrum-lcc-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-LCC_uncompressed.dmg

echo "Done."
md5sum electrum-lcc-$VERSION.dmg
